var JspecRails = {
  ...
  load_fixture: function(fixture_name) {
    ...
    if(JspecRails.fixture_cache[fixture_name]) {
      JspecRails.play_area.update(JspecRails.fixture_cache[fixture_name]);
    } else {
      new Ajax.Request('/jspec/fixture/' + fixture_name, 
       { parameters: { authenticity_token: JspecRails.rails_auth_token },
         asynchronous: false,
         onSuccess: function (transport) { JspecRails.fixture_cache[fixture_name] = transport.responseText;  
           JspecRails.load_fixture(fixture_name);
         } 
       } ); 
    }
  },
